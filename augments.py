...
augs = ["none", "blend", "cutmix"]
probs = [1.0, 0.0, 0.5]
alphas = [1.0, 0.7, 0.5]

def augment_sample(event_bin, hsi_blur, hsi_gt):

    c_1 = event_bin.shape[1]
    c_2 = hsi_blur.shape[1]
    c_3 = hsi_gt.shape[1]

    stacked_data = torch.cat((event_bin, hsi_blur, hsi_gt), dim=1)
    aug_data, _, _, aug_type = apply_augment(stacked_data, stacked_data, augs, probs, alphas)
    ...
    aug2 = aug_data[:,c_1:c_1+c_2,:,:]
    aug3 = aug_data[:,c_1+c_2:,:,:]
...
def blend(im1, im2, prob=1.0, alpha=0.6):
    ...
    c = torch.empty((im2.size(0), 1, 1, 1), device=im2.device).uniform_(0, 0.5)
    rim2 = c.repeat((1, im1.shape[1], im2.size(2), im2.size(3)))
    rim1 = c.repeat((1, im1.shape[1], im1.size(2), im1.size(3)))
